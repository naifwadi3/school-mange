...
class AddParent extends Component
{
    ...
    public function updated($propertyName)
    {
        $this->validateOnly($propertyName, [
            ...
            'Phone_Father' => 'nullable|string|min:10|max:10|regex:/^\d{1}-\d{4}-\d{3}$/',
            'National_ID_Mother' => 'required|string|min:10|max:10|regex:/[0-9]{9}/',
            'Passport_ID_Mother' => 'min:10|max:10',
            'Phone_Mother' => 'nullable|string|min:10|max:10|regex:/^\d{1}-\d{4}-\d{3}$/'
        ]);
    }
        ...
        try {
            ...
            if (!empty($this->photos)){
                foreach ($this->photos as $photo) {
                    $photo->store($this->National_ID_Father, $photo->getClientOriginalName(), $disk = 'parent_attachments');
